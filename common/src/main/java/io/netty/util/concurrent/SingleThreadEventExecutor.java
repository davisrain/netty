...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    private boolean fetchFromScheduledTaskQueue() {
        // 如果scheduledTaskQueue为null或者为空，直接返回
        ...
        for (;;) {
            // 根据当前时间从scheduledTaskQueue中获取到期的任务
            Runnable scheduledTask = pollScheduledTask(nanoTime);
            if (scheduledTask == null) {
                return true;
            }
            // 将scheduled任务添加到 任务队列 中，如果失败，将其再添加回scheduled任务队列
    ...
    final boolean offerTask(Runnable task) {
        // 如果eventExecutor的状态是大于SHUTDOWN的，直接拒绝
        if (isShutdown()) {
            reject();
        }
        // 否则将任务添加到任务队列中
    ...
    protected boolean runAllTasks(long timeoutNanos) {
        // 从scheduled任务队列中取出任务到 任务队列中
        fetchFromScheduledTaskQueue();
        // 从任务队列中获取出一个任务
        Runnable task = pollTask();
        // 如果取出的任务为null
        if (task == null) {
            // 调用afterRunningAllTasks方法，eventLoop的实现会取tailTaskQueue中再获取任务
            afterRunningAllTasks();
            return false;
        }

        // 如果传入的超时时间大于0，那么将deadline设置为当前时间+超时时间，否则，将deadline设置为0
        ...
        for (;;) {
            ...
            // 每执行64次任务 就检查一下超时时间，因为获取当前时间的操作相对来说比较昂贵
            if ((runTasks & 0x3F) == 0) {
                lastExecutionTime = getCurrentTimeNanos();
                // 如果最后执行时间大于了 deadline，跳出循环
        ...
        // 调用afterRunningAllTasks方法
        afterRunningAllTasks();
        // 将最后执行时间设置进eventExecutor中，返回
    ...
    private boolean runShutdownHooks() {
        ...
        // 执行shutdownHook任务，直到shutdownHooks为空
    ...
    public Future<?> shutdownGracefully(long quietPeriod, long timeout, TimeUnit unit) {
        ...
        // 如果当前EventExecutor的状态已经 大于 SHUTTINGDOWN 了
        if (isShuttingDown()) {
            // 返回自身的terminationFuture
            return terminationFuture();
        }

        // 判断当前线程是否是eventExecutor自身持有的线程
        boolean inEventLoop = inEventLoop();
        boolean wakeup;
        int oldState;
        for (;;) {
            // 如果状态大于 shutting down了
            if (isShuttingDown()) {
                // 返回terminationFuture
                return terminationFuture();
            }
            ...
            wakeup = true;
            oldState = state;
            // 如果当前线程是eventExecutor持有的线程
            if (inEventLoop) {
                // 直接设置新状态为shutting down
    ...
    protected boolean confirmShutdown() {
        // 如果状态不是大于 shutting down的，直接返回false
        if (!isShuttingDown()) {
            return false;
        }

        // 如果当前线程不是eventExecutor持有的线程，报错
        if (!inEventLoop()) {
        ...
        // 如果 gracefulShutdownStartTime 为0，设置为当前时间
        ...
        if (runAllTasks() || runShutdownHooks()) {
            // 如果eventExecutor的状态已经是shutdown 直接返回
            if (isShutdown()) {
            ...
            // 如果gracefulShutdownQuietPeriod为0的话，不进行等待任务队列中是否还有新任务进来，直接返回true
            if (gracefulShutdownQuietPeriod == 0) {
                return true;
            }
            // 否则的话，向任务队列中添加WAKEUP_TASK，并返回false，再等待一会儿
            taskQueue.offer(WAKEUP_TASK);
            return false;
        }
        ...
        // 如果没有任务要执行，状态已经大于等于shutdown 或者 当前时间减去 shutdown开始时间 已经大于 shutdownTimeout了，直接返回true
        if (isShutdown() || nanoTime - gracefulShutdownStartTime > gracefulShutdownTimeout) {
            return true;
        }

        // 如果当前时间 减去 最后执行时间 小于等于 shutdownQuietPeriod，那么向任务队列中添加WAKEUP_TASK，并且休眠100ms，返回false
        ...
        // 如果已经超过了 静默期，并且没有新的任务添加进来，那么返回true，真正的shutdown
        return true;
    }
    ...
    private void execute0(@Schedule Runnable task) {
        ObjectUtil.checkNotNull(task, "task");
        // 根据task是否是LazyRunnable类型的 以及 是否需要唤醒eventExecutor中的线程来决定是否立刻执行task
    ...
    private void execute(Runnable task, boolean immediate) {
        ...
        // 将任务添加到队列中
        addTask(task);
        // 如果当前线程不是eventExecutor持有的线程
        if (!inEventLoop) {
            // 调用startThread 开启eventExecutor的线程
            startThread();
            // 如果状态已经是shutdown
            if (isShutdown()) {
        ...
        // 如果addTaskWakesUp为false并且immediate为true，调用wakeup方法
    ...
    private void doStartThread() {
        // 判断自身持有的thread为null
        assert thread == null;
        // 调用自身持有的executor执行，默认会创建出一个FastThreadLocalThread来执行任务
        executor.execute(new Runnable() {
            @Override
            public void run() {
                // 将eventExecutor中的thread设置为执行该任务的thread
                thread = Thread.currentThread();
                // 如果interrupted标志为true，调用thread的interrupt方法中断
                if (interrupted) {
                    thread.interrupt();
                }

                boolean success = false;
                // 更新lastExecutionTime为当前时间
                updateLastExecutionTime();
                try {
                    // 调用SingeThreadEventExecutor的run方法
                    SingleThreadEventExecutor.this.run();
                    success = true;
                } catch (Throwable t) {
                    logger.warn("Unexpected exception from an event executor: ", t);
                } finally {
                    // 如果eventExecutor线程执行出现了异常，导致走到了finally
                    for (;;) {
                        // 将状态通过cas设置为SHUTTING_DOWN
                        int oldState = state;
